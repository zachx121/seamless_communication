...
class Translator(nn.Module):
    def __init__(
        ...
        logging.info(f"model_name_or_card retrieved: model_name_or_card={model_name_or_card}")
        # logging.info(f"model_name_or_card uri: {model_name_or_card.field("checkpoint").as_uri()}")
        ...
        logging.info(f"will execute 'load_unity_model', model_name_or_card={model_name_or_card}")
    ...
    def asr_predict(
        self,
        input: Union[str, Tensor, SequenceData],  # 输入的音频数据（路径、Tensor、或 SequenceData）
        tgt_lang: str,                           # 目标语言
        sample_rate: int = 16000,                # 音频采样率
        src_lang: Optional[str] = None,          # 源语言（如果需要）
        text_generation_opts: Optional[SequenceGeneratorOptions] = None,  # 文本生成选项
        unit_generation_opts: Optional[SequenceGeneratorOptions] = None  # 单元生成选项
    ) -> List[str]:
        """
        单独的 ASR 预测方法，将音频转换为文本
        """
        input_modality, output_modality = self.get_modalities_from_task_str(Task.ASR.name)
        ...
        if isinstance(input, dict):
            ...
            if isinstance(audio, str):
                ...
                if audio.dim() == 1:
                    ...
                    logger.warning("Transposing audio tensor from (bsz, seq_len) -> (seq_len, bsz).")
                    audio = audio.transpose(0, 1)

                decoded_audio = {"waveform": audio, "sample_rate": sample_rate, "format": -1}
            src = self.collate(self.convert_to_fbank(decoded_audio))["fbank"]
